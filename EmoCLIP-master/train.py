...
def evaluate(loader, model, cnf):
    """
    Evaluate the model and compute metrics using EmoCLIP's rich class descriptions
    """
    model.eval()
    all_predictions = []
    all_labels = []
    ...
    # Tokenize all class descriptions once
    class_tokens = clip.tokenize(class_descriptions, context_length=77, truncate=True).to(cnf.device)
    ...
    for batch_idx, data in enumerate(loader):
        ...
        # Forward pass using similarity mode with class descriptions
        with autocast():
            logits_per_image, _ = model(inputs, class_tokens, mode='similarity')
            
            # Contrastive evaluation loss (optional)
            loss = nn.CrossEntropyLoss()(logits_per_image, labels)
            val_loss += loss.item()
            
            # Get predictions by finding the highest similarity class
